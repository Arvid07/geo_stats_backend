...
pub struct GameData {
    ...
    pub insert_players: Vec<PlayerModel>,
    pub update_players: Vec<PlayerModel>,
    pub insert_comp_teams: Vec<CompTeamModel>,
    pub update_comp_teams: Vec<CompTeamModel>,
    pub insert_fun_teams: Vec<FunTeamModel>,
    pub map: MapModel
}
...
pub async fn create_new_player_model(player_id: &str, client: &Client) -> Result<PlayerModel, Error> {
...
async fn get_player_model(player_id: &str, db: &DatabaseConnection, client: &Client) -> Result<Result<PlayerModel, PlayerModel>, Error> {
...
pub async fn get_game_data(
    ...
    let game = client.get(format!("https://game-server.geoguessr.com/api/duels/{}", game_id))
        ...
        .await
        .map_err(|_| {
            error!("Could not find Game with id: {}!", game_id);
            ErrorBadRequest(format!("Could not find Game with id: {}!", game_id))
        })?;

    if game.status.as_str() != "Finished" {
        error!("Game has not finished yet! {}", game_id);
        ...
        .enumerate()
    {
        ...
        for (i, team) in game.teams.iter().enumerate() {
            for player in team.players.iter() {
                let geo_guess_option = player.guesses.iter().find(|guess| {
                    guess.round_number as usize - 1 == round_number
                });
                ...
                if let Some(geo_guess) = geo_guess_option {
                    ...
                    let score = geo_guess.score.unwrap_or_else(|| {
                        let max_distance = max_distance_option.unwrap_or_else(|| {
                            let a = geoutils::Location::new(game.map_bounds.min.lat, game.map_bounds.min.lng);
                            let b = geoutils::Location::new(game.map_bounds.max.lat, game.map_bounds.max.lng);
...
async fn insert_duels_game(
    ...
    match db.transaction::<_, _, DbErr>(|txn| {
        Box::pin(async move {
            DuelsGame::insert(game_data.duels_game).exec(txn).await?;
            
            if !game_data.guesses.is_empty() {
                Guess::insert_many(game_data.guesses).exec(txn).await?;
            }
            if !game_data.rounds.is_empty() {
                DuelsRound::insert_many(game_data.rounds).exec(txn).await?;
            }
            if !game_data.insert_players.is_empty() {
                Player::insert_many(game_data.insert_players).exec(txn).await?;
            }
            for player in game_data.update_players {
                player.update(txn).await?;
            }
            if !game_data.insert_comp_teams.is_empty() {
                CompTeam::insert_many(game_data.insert_comp_teams).exec(txn).await?;
            }
            for team in game_data.update_comp_teams {
                team.update(txn).await?;
            }
            if !game_data.insert_fun_teams.is_empty() {
                FunTeam::insert_many(game_data.insert_fun_teams).exec(txn).await?;
            }
    ...
    let _ = Map::insert(game_data.map).exec(db).await;

    for location in game_data.locations {
        let _ = location.update(db).await;
    }
...
async fn insert_solo_game(
    ...
    for location in locations {
        let _ = Location::insert(location).exec(db).await;
    }
